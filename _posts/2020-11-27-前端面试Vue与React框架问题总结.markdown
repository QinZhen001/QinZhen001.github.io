---
layout:     post
title:      "前端面试：Vue 与 React 框架问题总结"
description: "整理「前端面试：Vue 与 React 框架问题总结」相关的高频问题、核心知识点、答题思路和复盘要点。"
...
> 更新说明：本篇 Vue 部分主要基于 Vue 2 响应式（`Object.defineProperty`）和虚拟 DOM，React 部分主要基于 React 16 的生命周期、Fiber 和 setState 批处理。这些原理仍值得理解，但当前面试建议补充：Vue 3 的 Proxy 响应式、Composition API、`<script setup>`、编译期优化、Pinia；React 的 Hooks、并发特性、Server Components、Actions（`useActionState`/`useOptimistic`）和 React Compiler。回答时最好点明“Vue2/React16 是这样，Vue3/React18+ 有何变化”。
...
1. **小型应用**：如果你的应用较小，状态管理的需求相对简单，Context API 足以满足大多数需求。
2. **需要跨组件传递状态**：如果你需要在多个组件之间共享一些简单的状态（比如主题、认证状态等），Context 是一个理想的选择。
3. **没有复杂的状态逻辑**：当状态更新的逻辑不复杂时（比如简单的值，或仅需要在父子组件之间传递状态），Context 会更容易实现。
4. **易于实现**：Context API 相对简单，适合快速原型开发或简化项目的状态管理。

使用全局状态管理库的场景

1. **中大型应用**：当应用变得复杂，需要处理多个状态切片以及它们之间的逻辑关系时，全局状态管理库可以提供更好的组织和可维护性。
2. **复杂的状态逻辑**：如果你需要对状态进行复杂的操作，比如异步请求、多个状态依赖或需要优化性能的更新，使用 Redux 等库会更为高效。
3. **调试工具和开发支持**：Redux 等状态管理库通常提供良好的调试工具和中间件，可以帮助你更好地跟踪和管理状态变化。
4. **团队协作**：如果多个开发者在同一项目中工作，使用状态管理库会对代码的可理解性和维护性有很大帮助。
