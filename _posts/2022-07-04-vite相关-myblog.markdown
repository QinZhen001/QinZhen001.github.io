...
Vite 早期开发阶段依赖 esbuild 做依赖预构建，生产构建主要依赖 Rollup。随着 Rolldown 集成推进，新版本逐步使用 Rust 实现的 Rolldown 统一构建链路，目标是降低开发和生产构建之间的差异，并提升大型项目构建性能。

升级时需要重点关注：

- 插件兼容性：大部分 Rollup 风格插件可以复用，但依赖内部实现细节的插件需要单独验证。
- CommonJS / ESM 边界：历史依赖的格式转换可能暴露更多兼容问题。
- 构建产物差异：升级前后要对 chunk 数量、文件名、动态导入和 CSS 抽取结果做对比。
- CI 缓存：升级构建引擎后，重新评估依赖缓存和产物缓存策略。
