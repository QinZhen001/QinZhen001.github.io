...
layout:     post
title:      "网络基础知识总结"
description: "本文整理《网络基础知识总结》相关的核心知识点、实践经验与参考资料。"
...
> ⚠️ 注意：103 Early Hints 不是 HTTP/3 特有的，它可以在 HTTP/1.1、HTTP/2 和 HTTP/3 中使用！

103 Early Hints 是一个 HTTP 状态码，用于在服务器准备最终响应之前，提前告知浏览器哪些资源可以预加载。

```
客户端 → GET /index.html                    → 服务器

服务器开始处理请求...
服务器知道页面需要 style.css 和 script.js

服务器 ← HTTP/1.1 103 Early Hints          ← 服务器 (提前发送)
...
// 4. 浏览器解析 HTML，发现需要 CSS 和 JS
// 5. 才开始请求这些资源
GET /style.css
GET /script.js

问题：
浏览器在服务器处理期间（200ms）什么都不做 ❌
等待时间被浪费了 ❌
```
...
// 2. 服务器立即发送 103 Early Hints (耗时 1ms)
103 Early Hints
Link: </style.css>; rel=preload; as=style
Link: </script.js>; rel=preload; as=script
...
每个域名（hostname）通常限制为 6-8 个并发连接：

这是因为 HTTP/1.1 协议的限制，浏览器需要为每个请求建立单独的 TCP 连接。
...
支持多路复用（Multiplexing），可以在单个 TCP 连接上同时传输多个请求和响应：

- 理论上没有严格的并发连接数限制

- 所有请求都在一个连接上复用

- 默认最大并发流数量通常为 100-128 个

- 性能大幅提升，避免了 HTTP/1.1 的队头阻塞问题
...
**注意：Chrome 已经完全移除了对 HTTP/2 Server Push 的支持。**

结论：HTTP/2 Server Push 已死，拥抱 103 Early Hints！ 🎉
...
- TCP 层面的队头阻塞：虽然 HTTP/2 在应用层通过多路复用解决了 HTTP/1.1 的队头阻塞，但在 TCP 层仍然存在

- 当 TCP 连接中某个数据包丢失时，整个连接上的所有流都会被阻塞，必须等待丢包重传

- 即使其他流的数据已经到达，也无法被应用层处理

```
HTTP/2 多路复用示意：
Stream 1: [数据1] [数据2] [数据3✗丢包] [数据4]
Stream 2: [数据A] [数据B] [数据C] [数据D✓已到达]
Stream 3: [数据X] [数据Y] [数据Z✓已到达]

问题：虽然 Stream 2 和 Stream 3 的数据都已到达，
但由于 TCP 需要保证顺序，必须等待 Stream 1 的丢包重传
```
...
- 再加上 TLS 1.2 握手（2 RTT）或 TLS 1.3 握手（1 RTT）
...
- HTTP/2 基于 TCP，连接由四元组标识（源 IP、源端口、目标 IP、目标端口）

- 当网络切换时（如 WiFi ↔ 4G），IP 地址改变会导致连接断开

- 需要重新建立连接（TCP 三次握手 + TLS 握手），延迟较高
...
特别适合：移动应用、弱网环境、需要低延迟的场景（如实时支付、游戏、视频）
...
```bash
QUIC 多路复用示意：
Stream 1: [数据1] [数据2] [数据3✗丢包] [数据4]  ← 仅此流被阻塞
Stream 2: [数据A] [数据B] [数据C] [数据D✓]      ← 可以正常处理
Stream 3: [数据X] [数据Y] [数据Z✓]              ← 可以正常处理

解决方案：QUIC 在协议层面实现了独立的流控制，
每个流的丢包只影响该流本身，其他流可以继续传输
```

关键特性：

- 基于 UDP，没有 TCP 的顺序保证限制
...
```
// HTTP/2: 连接由四元组标识
TCP Connection = (源IP, 源端口, 目标IP, 目标端口)
// 网络切换 → IP 变化 → 连接断开

// HTTP/3: 连接由 Connection ID 标识
QUIC Connection = Connection ID (64位唯一标识)
// 网络切换 → IP 变化 → 连接保持 ✓
```

优势：

- 使用 Connection ID 标识连接，与 IP/端口无关

- 网络切换时（WiFi ↔ 移动网络），连接无缝迁移
...
客户端 → SYN           → 服务器    (1 RTT)
客户端 ← SYN+ACK       ← 服务器
客户端 → ACK + TLS     → 服务器    (1 RTT)
客户端 ← TLS Finished  ← 服务器
客户端 → HTTP Request  → 服务器    (1 RTT)
总计: 3 RTT

HTTP/3 (QUIC):
首次连接:
客户端 → Initial + TLS → 服务器    (1 RTT)
客户端 → HTTP Request  → 服务器    (1 RTT)
总计: 2 RTT，但可以在握手时发送部分数据

后续连接 (0-RTT):
客户端 → 0-RTT + HTTP Request → 服务器
总计: 0 RTT ✓ (利用之前的会话密钥)
```
...
结果：
HTTP/2: CPU 使用率 100% (基准)
HTTP/3: CPU 使用率 120-150% ❌
```
...
移动设备:
HTTP/2: 电池消耗 100mAh/小时
HTTP/3: 电池消耗 115mAh/小时 ❌
- 移动设备电池续航下降
```
...
```
中间设备 (Middleboxes) 对 UDP 的处理不如 TCP 成熟
...
3. QoS (服务质量)
   ...
   - 给 TCP 更高优先级
   - UDP 被当作"不重要"流量
   - 造成 HTTP/3 比 HTTP/2 慢 ❌

4. NAT/防火墙
   虽然 QUIC 对 NAT 更友好，但:
   - 某些 NAT 对 UDP 的超时时间更短
   - UDP 状态表可能更小
   - 大量 UDP 连接可能被限制
```
...
Node.js: 
- quic (实验性) ⚠️
- @fails-components/webtransport ⚠️
...
QUIC（Quick UDP Internet Connections）是**一种实验性传输层网络协议**，提供与TLS/SSL相当的安全性，同时具有更低的连接和传输延迟。 QUIC基于UDP，因此拥有极佳的弱网性能，在丢包和网络延迟严重的情况下仍可提供可用的服务。
...
CNAME 转发是一种基于 DNS 记录的别名映射技术，通过将域名解析请求**间接转发到目标域名**，实现灵活的资源管理和服务优化。以下是其核心原理与应用场景的详细解析：

**核心原理**

**别名映射机制**
CNAME 记录在 DNS 系统中创建别名关系，例如将 `shop.example.com` 指向 `cdn.provider.com`。当用户访问 `shop.example.com` 时，DNS 服务器会先解析该 CNAME 记录，获取目标域名 `cdn.provider.com`，再重新启动解析流程，最终获取目标 IP 地址

**解析流程**：`用户请求 → 解析 CNAME 记录 → 获取目标域名 → 继续解析目标域名的 A 记录 → 返回 IP 地址`

**链式解析特性**

CNAME 支持多级转发（如 A→B→C），但需避免循环依赖（如 A→B→A），否则会导致解析失败
...
将用户域名通过 CNAME 指向 CDN 服务商提供的域名（如 `example.cdn.com`），CDN 根据用户地理位置返回就近节点的 IP 地址，提升访问速度

```
www.example.com  CNAME  example.cdn.com
example.cdn.com  A      1.2.3.4 (CDN节点IP)
```

**网站重定向与服务迁移**

* **旧域名跳转**：将旧域名（如 `old.com`）指向新域名（如 `new.com`），实现无缝跳转

* **服务切换**：在不改变用户访问习惯的情况下，将服务迁移至新服务器或平台

**负载均衡与故障转移**

* 通过多个 CNAME 记录指向不同服务器，结合 DNS 轮询策略，实现流量分发

* 当某台服务器宕机时，自动将请求转发至备用服务器，保障服务连续性

**子域名统一管理**

将多个子域名（如 `blog.example.com`、`api.example.com`）统一指向主域名或第三方服务，简化配置流程
...
SSL（安全套接层）是一种用于保护网络通信安全的协议，它的核心使命是在客户端（比如你的浏览器）和服务器（比如你访问的网站）之间建立一个**加密的通道**，确保传输的数据无法被窃听和篡改，同时对服务器进行身份认证 

| 特性         | 描述                                                         |
| ------------ | ------------------------------------------------------------ |
| **全称**     | Secure Sockets Layer（安全套接层）                           |
| **主要目标** | 保证数据传输的**机密性**、**完整性**，并对服务器进行**身份认证** |
| **工作层**   | 位于**应用层**（如HTTP）和**传输层**（TCP）之间              |
| **关键机制** | 非对称加密交换密钥、对称加密传输数据、数字证书验证身份、消息验证码（MAC）保证完整性 |
| **核心协议** | 握手协议、记录协议、警报协议等                               |
| **现状**     | 已被更安全、更现代的**TLS**（传输层安全）协议取代，但“SSL”一词仍被广泛使用 |
...
> 由于SSL 3.0被发现存在安全缺陷，互联网工程任务组将其标准化并进行了改进，推出了其继任者，并命名为**TLS**。可以理解为，TLS 1.0就是SSL 3.1

TLS 协议的全称是**传输层安全性协议**（Transport Layer Security）

TLS 是互联网上最为重要的安全协议之一，它就像一位尽职尽责的私人信使，确保您在网络上传输的数据是**加密的、完整的，并且送达了正确的对象**。它位于可靠的传输协议（如 TCP）之上，负责对上层协议（如 HTTP）传送的数据进行**分块、压缩、添加消息验证码、加密**，然后传输

TLS 主要为实现三个关键的安全目标：

- **保密性**：通过加密技术，确保通信内容只有发送方和预期的接收方能够解读，对窃听者来说如同乱码 。
- **完整性**：通过哈希函数等机制，检测数据在传输过程中是否被篡改，一旦被修改，接收方能够立即发现 。
- **身份认证**：通常利用基于 X.509 的数字证书来验证服务器的身份，确保您连接的是真实的网站或服务，而非冒充的钓鱼网站 。在某些需要高安全性的场景下，也可以进行客户端的身份认证，实现双向验证 



### 相关版本

| 版本        | 发布年份 | 状态与核心特点                                               | 关键改进与差异                                               |
| :---------- | -------- | :----------------------------------------------------------- | ------------------------------------------------------------ |
| **TLS 1.0** | 1999     | **已废弃**。作为 SSL 3.0 的轻微升级，奠定了 TLS 协议的基础   | 引入基于标准 HMAC 的 PRF，改进了 Finished 报文和填充格式     |
| **TLS 1.1** | 2006     | **已废弃**。主要是一个安全维护版本，修复了 TLS 1.0 的关键漏洞 | 针对 CBC 模式引入**显式初始化向量（IV）**，以防御 BEAST 等攻击 |
| **TLS 1.2** | 2008     | **当前主流和广泛使用**的版本。支持现代加密算法，灵活性高     | 引入对 **SHA-256** 等更强哈希算法的支持；增加对 **AEAD** 加密模式（如 AES-GCM）的支持，提升了安全性和效率 |
| **TLS 1.3** | 2018     | **最新、最快、最安全**的版本。经过彻底简化和现代化设计       | **简化握手**（可达到 1-RTT），**强制前向保密**，**移除了大量不安全算法**（如 RSA 密钥交换、SHA-1、CBC 模式），仅保留 AEAD 模式密码套件 |

**安全性与算法的精简**：TLS 1.3 在安全性上做出了最激进的改进。它移除了 TLS 1.2 及之前版本中许多被证实存在安全隐患或不再推荐的特性，例如静态 RSA 密钥交换（无法提供前向保密）、CBC 模式加密、RC4、SHA-1 等 。这使得协议更简洁，攻击面更小。同时，TLS 1.3 **强制要求使用前向保密**，这意味着即使服务器的长期私钥在未来泄露，过去的通信记录也不会被解密 。

**性能优化：握手延迟的显著降低**：一个非常重要的改进是握手速度。在 TLS 1.2 及之前版本中，完整的握手通常需要 **2次往返（2-RTT）**。而 TLS 1.3 通过将密钥交换过程与客户端初始消息合并，将握手减少到仅需 **1次往返（1-RTT）**，显著降低了连接建立的延迟 。此外，TLS 1.3 还支持 **0-RTT 模式**，对于近期连接过的服务器，客户端可以在第一次消息中就携带应用数据，进一步加快速度，但需注意此模式可能存在重放攻击的风险 



### 支持前向保密

| 特性         | 不支持前向保密（如 TLS 1.2 中的 RSA 密钥交换）               | 支持前向保密（TLS 1.3 强制使用，如 ECDHE）                   |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 核心机制     | 使用服务器的**静态长期私钥**直接加密生成会话密钥。           | 使用**临时密钥对**通过算法（如ECDHE）现场协商出会话密钥。    |
| 密钥关联性   | 会话密钥与服务器的长期私钥**直接相关**。                     | 会话密钥仅与本次握手产生的临时密钥**相关**。                 |
| 私钥泄露后果 | **灾难性的**：长期私钥泄露会导致所有被存储的历史通信记录被解密。 | **影响可控**：长期私钥泄露不影响过去会话的安全，因为解密所需的临时私钥并未保存。 |
| 形象比喻     | 用一把**万能钥匙**（长期私钥）锁上所有的保险箱，丢了钥匙就丢了一切。 | 每次会话都用**一次性密码锁**（临时密钥），开完即废，互不干扰。 |

**为什么前向保密至关重要**：这主要是为了应对“**今天截获，日后破解**”的攻击模式。一个有能力监控网络流量的攻击者，可以大规模地存储加密的网络通信数据。如果加密方案不支持前向保密，那么一旦他未来通过某种手段（如入侵服务器）获取了服务器的长期私钥，他就可以用这把钥匙轻松解密所有之前截获的“密文宝藏”。而前向保密机制彻底杜绝了这种可能性，极大地提高了数据保护的长期安全性，符合如支付卡行业数据安全标准（PCI DSS）等合规性要求中关于保护传输数据的要求
...
NAT（Network Address Translation，网络地址转换） 是一种在路由器上运行的网络技术，用于将私有 IP 地址转换为公网 IP 地址。
...
```
// 内网设备访问外网服务器
场景：电脑 (192.168.1.100) 访问 Google (142.250.185.46:443)

1. 电脑发送数据包：
   源地址: 192.168.1.100:54321  ← 私有 IP + 随机端口
   目标地址: 142.250.185.46:443

2. 路由器 (NAT) 修改数据包：
   源地址: 203.0.113.10:12345   ← 公网 IP + NAT 映射端口
   目标地址: 142.250.185.46:443
...
3. Google 服务器收到：
   看到的源地址: 203.0.113.10:12345  ← 只知道公网 IP
   
4. Google 回复：
   源地址: 142.250.185.46:443
   目标地址: 203.0.113.10:12345

5. 路由器 (NAT) 查表转换：
   查找 NAT 表: 12345 → 192.168.1.100:54321
   修改目标地址为: 192.168.1.100:54321
...
```
场景：你想在家里搭建一个 Web 服务器

内网服务器: 192.168.1.100:8080
公网 IP:    203.0.113.10

外部用户尝试访问: 203.0.113.10:8080
路由器: "8080 端口？我的 NAT 表里没有这个映射！"
结果: 连接失败 ❌
...
公网: 203.0.113.10                       公网: 198.51.100.20
         │                                       │
         │  A 想直接连接 B                         │
         │  但 B 在 NAT 后面                       │
         │  无法直接访问 10.0.0.50 ❌              │
...
移动设备 → 建立连接 → 服务器
NAT 表添加映射: 192.168.1.100:54321 ↔ 203.0.113.10:12345

15 分钟后...
移动设备休眠，没有数据传输

NAT 表: "15 分钟没活动，删除这个映射"
NAT 映射被删除 ❌

移动设备唤醒，尝试通信
服务器发送数据 → 203.0.113.10:12345
路由器: "12345？没有这个映射记录！"
数据包被丢弃 ❌
...
NAT 穿透（NAT Traversal） 是指在 NAT 环境下，让外部设备能够主动连接到内网设备的技术。
...
* 不同 adId  保证对应不同的事件回调
* hippy/h5 中 广告回调 data 保持数据一致性
* event 回调中对齐现有web业务，不直接采用native event 枚举
...
- **衡量网络延迟**：RTT是网络延迟的直接体现。较低的RTT通常意味着响应迅速，用户体验更好；而较高的RTT则可能表示网络拥塞或路径不佳。
- **保障传输可靠性**：在像TCP这样的可靠传输协议中，RTT是决定**超时重传时间（RTO）** 的基础。发送方会为每个数据包启动一个重传计时器，如果在这个时间内没有收到确认，就会重新发送数据包
