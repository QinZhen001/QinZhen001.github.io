---
layout:     post
title:      "TypeScript 类型系统与工程实践总结"
description: "系统整理「TypeScript 类型系统与工程实践总结」的语言特性、底层机制、代码示例和实践经验。"
...
这篇文档已经覆盖了 TypeScript 的大部分常用知识点，但原结构更像零散笔记。建议按“基础语法 → 类型系统 → 类型编程 → 工程化配置 → 框架实践 → 常见错误”来阅读和维护：

1. **基础语法**：`interface`、`type`、`enum`、`class`、`readonly`、`namespace`、`declare`。
2. **类型运算与收窄**：`keyof`、`typeof`、`extends`、`infer`、`in`、`as`、`is`、`asserts`、联合/交叉类型。
3. **泛型与类型编程**：泛型约束、条件类型、分发、协变/逆变、模板字符串类型、工具类型。
4. **工程化配置**：`tsc`、`tsconfig`、声明文件、模块解析、Babel/Webpack 集成。
5. **React 与业务实践**：React 类型、请求返回值类型、字典类型、类实例类型等。
6. **常见错误排查**：按 `ts(xxxx)` 错误码归档，保留“原因 → 示例 → 解决方案”的结构。

> 维护建议：后续新增内容优先放入上述分类，不要继续追加到“补充”末尾；错误类内容统一放到“常见错误与排查”。

### 工程实践建议

TypeScript 项目不要只关注语法，还要关注编译边界和类型检查边界：

- 应用项目：建议开启 `strict`，构建工具负责转译，`tsc --noEmit` 负责类型检查。
- 库项目：重点关注 `declaration`、`exports`、`types`、`moduleResolution`、`verbatimModuleSyntax` 和声明文件产物。
- Monorepo：优先使用 project references 或按包独立类型检查，避免单个根 `tsconfig` 越来越慢。
- Node 项目：根据运行时选择 `module: node20` / `nodenext` / `bundler`，不要照搬浏览器项目配置。
- 前端项目：如果使用 Vite/Rspack/Webpack 转译 TS，要明确它们通常只做转译，不等价于完整类型检查。

推荐把 `tsconfig` 拆成基础配置和场景配置，例如 `tsconfig.base.json`、`tsconfig.app.json`、`tsconfig.node.json`，避免所有环境共用一份配置。

### TypeScript 5.x / 6.x / 7.x 对比与发布时间

> 日期主要参考 Microsoft TypeScript 官方博客公告；npm / GitHub tag 的发布时间可能因为时区、补丁版本或发布流程存在 1 天左右差异。

#### 大版本定位对比

| 大版本 | 发布时间范围 | 当前定位 | 核心变化 | 升级关注点 |
| --- | --- | --- | --- | --- |
| TypeScript 5.x | 2023-03 ~ 2025-08 | 现有 JS/TS 编译器实现的成熟演进版本 | 标准装饰器、`const` 类型参数、`satisfies` 生态完善、`moduleResolution: bundler`、`verbatimModuleSyntax`、`NoInfer`、`isolatedDeclarations`、`noCheck`、`import defer`、`node20` 等 | 主要是类型检查更严格、模块解析更现代、声明文件生成能力增强 |
| TypeScript 6.x | 2026-02 ~ 2026-04 | JS/TS 编译器实现的最后一个大版本，面向 TypeScript 7 的过渡版本 | 新默认值、`es2025`、Temporal 类型、`stableTypeOrdering`、`#/` subpath imports、清理旧配置 | 旧配置大量弃用，建议先用 6.x 清理项目，为 7.x 迁移做准备 |
| TypeScript 7.x | 2026-04 起 | Go 原生实现的新编译器 / 语言服务版本 | 8x~12x 构建加速、LSP 语言服务、多线程 `--checkers` / `--builders`、重写 watch 模式 | 7.0 暂无稳定 programmatic API，依赖 `typescript` API 的工具链可能需要继续并行保留 6.x |

#### 5.x 发布时间线

| 版本 | 官方公告日期 | 重点内容 |
| --- | --- | --- |
| 5.0 | 2023-03-16 | 标准 Decorators、`const` 类型参数、`extends` 支持多个配置、所有 enum 成为 union enum、`moduleResolution: bundler`、`verbatimModuleSyntax` |
| 5.1 | 2023-06-01 | 返回类型推断改进、JSX 类型检查优化、命名空间 JSX 属性支持、编辑器体验优化 |
| 5.2 | 2023-08-24 | Explicit Resource Management、Decorator Metadata、元组标签改进、数组联合方法调用优化 |
| 5.3 | 2023-11-20 | Import Attributes、`resolution-mode`、`switch(true)` 收窄、布尔比较收窄、实例字段 `super` 检查 |
| 5.4 | 2024-03-06 | 闭包中保留类型收窄、`NoInfer<T>`、`Object.groupBy` / `Map.groupBy`、模块解析改进 |
| 5.5 | 2024-06-20 | 推断类型谓词、JSDoc `@import`、正则语法检查、Set 新方法、`isolatedDeclarations` |
| 5.6 | 2024-09-09 | Iterator Helper、总是真值/空值表达式检查、`--noCheck`、`--noUncheckedSideEffectImports`、`--build` 行为变化 |
| 5.7 | 2024-11-22 | 未初始化变量检查、`--rewriteRelativeImportExtensions`、`target/lib es2024`、Node V8 compile cache |
| 5.8 | 2025-02-28 | return 表达式分支细粒度检查、`module node18`、`module nodenext` 支持 CJS `require()` ESM、`--erasableSyntaxOnly`、`--libReplacement` |
| 5.9 | 2025-08-01 | 更精简的 `tsc --init`、`import defer`、`--module node20`、DOM API 摘要、可展开 hover、类型实例化缓存 |

#### 6.x 发布时间线

| 版本 | 发布时间 | 状态 | 重点内容 |
| --- | --- | --- | --- |
| 6.0 Beta | 2026-02-11 | Beta | 作为 5.9 到 7.0 的迁移桥梁，提前暴露默认值变化和弃用项 |
| 6.0 RC | 2026-03-06 | RC | 迁移验证版本，继续对齐 7.0 行为 |
| 6.0 | 2026-03-23 | Stable | 最后一个基于 JS/TS 代码库的大版本，默认值更现代，清理旧模块系统和旧编译目标 |
| 6.0.3 | 2026-04-16 | Patch | 6.0 稳定补丁版本 |

#### 7.x 发布时间线

| 版本 | 发布时间 | 状态 | 重点内容 |
| --- | --- | --- | --- |
| 7.0 Beta | 2026-04-21 | Beta | Go 原生编译器公开测试，验证性能和兼容性 |
| 7.0 RC | 2026-06-18 | RC | 发布候选版本，面向生产迁移前验证 |
| 7.0 | 2026-07-08 | Stable | Go 原生实现正式发布，进入 native TypeScript 工具链时代 |

#### 5.x / 6.x / 7.x 迁移建议

1. **普通业务项目**：可以先升级到 5.9，再评估 6.0 的默认值和弃用项，最后再尝试 7.0。
2. **库项目**：重点关注 `moduleResolution`、`exports/imports`、声明文件输出、`verbatimModuleSyntax` 和 `isolatedDeclarations`。
3. **老项目**：如果仍依赖 `target: es5`、`moduleResolution: node/classic`、`module: amd/umd/systemjs/none`、`baseUrl` 等旧配置，应先在 6.x 阶段清理。
4. **依赖 TypeScript API 的工具链**：TypeScript 7.0 暂不提供稳定 programmatic API，`typescript-eslint`、Volar、Vue/Svelte/Astro/Angular 相关工具需要关注兼容状态。
5. **大型 monorepo**：7.x 可以重点评估 `--checkers` 和 `--builders`，但要注意 CPU 与内存占用。
...
```typescript
const fn = (info: string | null | undefined) => {}
```
...
`Awaited<T>` 用来递归提取 `Promise` 或类 `Promise` 类型最终 resolve 出来的值类型，适合描述 `await` 或 `.then()` 后的结果。
...
当类的构造函数被声明为 `protected` 时，该类**无法通过 `new` 关键字在外部直接创建实例**，但允许子类继承并调用父类构造函数。
...
// 尝试直接实例化 → 报错
const base = new Base("001"); // ❌ 错误：类“Base”的构造函数是受保护的，仅可在类声明中访问

class Sub extends Base {
  constructor(id: string) {
    super(id); // ✅ 子类可调用 protected 构造函数
  }
}
const sub = new Sub("002"); // ✅ 允许通过子类实例化
```

该设计常用于**定义抽象基类或框架扩展点**，要求用户必须通过继承并实现特定逻辑才能使用功能
...
在 `tsconfig.json` 文件中启用 `allowImportingTsExtensions` 选项：
...
模板字符串类型可以在类型层面对字符串字面量进行拼接、组合和约束。它通常会和联合类型、`keyof`、条件类型一起使用。

```ts
type EventName<T extends string> = `${T}Changed`
type UserEvent = EventName<'name' | 'age'> // 'nameChanged' | 'ageChanged'
```
...
type UserGetter = Getter<'name' | 'age'> // 'getName' | 'getAge'
```

内置字符串工具类型包括：`Uppercase<T>`、`Lowercase<T>`、`Capitalize<T>`、`Uncapitalize<T>`。
...
主要用于优化 TypeScript 编译生成的 JavaScript 代码，提升项目性能和可维护性
...
在 TypeScript 中，`PropertyKey` 是一个联合类型，它表示一个对象的键，可以是 `string`、`number` 或 `symbol`。有时候，你可能希望限制对象的键类型，以确保键符合某些特定的类型约束。以下是一些常见的情况，以及如何在 TypeScript 中限制键的类型。

限制对象的键类型为特定类型

假设你有一个对象类型，你希望对象的键类型限制为特定的类型，比如 `string` 或 `number`。你可以使用 TypeScript 的映射类型和 `keyof` 操作符来实现。
...
在 TypeScript 中，可以使用映射类型和 `keyof` 操作符来限制对象的键类型。你可以限制键类型为特定的 `string`、`number` 或 `symbol`，或者结合 `PropertyKey` 来实现更灵活的键类型限制。希望这些示例能够帮助你在 TypeScript 中更好地限制对象的键类型。
