---
layout:     post
title:      "移动端 H5 适配与开发实践总结"
description: "围绕「移动端 H5 适配与开发实践总结」整理核心知识点、实践经验、常见问题和参考资料，便于持续查阅。"
...
移动端 H5 适配已经从早期 `rem + flexible` 逐步转向更原生的 viewport、动态视口单位和安全区适配。阅读旧方案时要区分“历史兼容方案”和“当前推荐方案”。

实践建议：

- 页面整体布局优先使用 Flex/Grid + 百分比/viewport 单位，不要所有尺寸都机械转换。
- 移动端高度优先考虑 `100dvh`、`100svh`、`100lvh`，减少地址栏显示/隐藏导致的 `100vh` 问题。
- 刘海屏和底部 Home Indicator 使用 `env(safe-area-inset-*)` 处理安全区。
- 图片优先使用 `srcset`、`picture`、WebP/AVIF 和合适的 DPR 资源，而不是一味使用大图。
- 适配方案要结合设计稿、业务类型和目标浏览器，不建议在新项目继续默认引入 `lib-flexible`。
...
Deferred Deeplink（延迟深度链接）是一种用于移动应用推广的高级技术，主要解决用户未安装目标应用时的精准跳转问题。其核心原理是：当用户点击推广链接时，系统会先检测设备是否已安装目标应用——若已安装则直接跳转至应用内指定页面；若未安装，则引导用户下载应用，并在首次启动时自动跳转到预设页面（如商品详情、活动页等），而非默认首页。以下是详细解析：

1. **用户行为触发**
   用户通过广告、社交分享、短信等渠道点击推广链接（例如电商商品页、活动页链接）。

2. **安装状态检测**

   - 已安装：直接唤醒应用并跳转至目标页面（如商品页product/123）。

   - 未安装：

     → 引导用户跳转至应用商店下载；

     → 下载安装后首次启动时，自动还原原始链接意图，跳转至目标页面。

3. **用户匹配技术**
   为实现“下载后精准跳转”，需解决​**​设备标识匹配​**​问题：

   - 传统方案：收集设备特征（IP、屏幕尺寸、系统版本等）生成唯一标识，服务端匹配用户点击与安装行为。
   - **优化方案**（iOS 9+）：通过 `SFSafariViewController` 共享 Safari 的 Cookie，实现 100% 精准匹配（但受限于微信等平台无法使用）

---

Deferred Deeplink 是移动增长领域的“黑科技”，通过 **安装状态检测 → 下载引导 → 意图还原** 三步闭环，破解了“未安装用户无法精准直达”的痛点。其价值在社交裂变、广告投放、用户召回等场景尤为显著，但需权衡开发成本与隐私合规要求。对于中小团队，建议优先接入 Branch 或 OpenInstall 等第三方服务，快速实现技术落地

### AppsFlyer

AppsFlyer 是一款全球领先的**移动应用归因与营销分析平台**，专注于帮助开发者、营销人员和企业精准追踪用户行为、优化广告投放效果并提升投资回报率（ROI）。以下是其核心解析：

AppsFlyer 是移动营销领域的“数据中枢”，通过**归因分析 → 行为追踪 → 深度报告**的闭环，解决“广告钱花在哪了？用户从哪来？如何提升收益？”三大核心问题。其价值在出海营销、用户增长和精细化运营场景中尤为显著，但需注意隐私合规与技术适配成本。对于中小团队，可优先使用其基础版SDK与免费报告功能；大型企业则可结合高级功能构建全链路数据体系
...
> 原文：fastClick原理分析（2020-10-21 16:57:00）
