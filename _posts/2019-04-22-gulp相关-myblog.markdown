---
layout:     post
title:      "Gulp 自动化构建实践总结"
description: "系统整理「Gulp 自动化构建实践总结」的配置方法、工作机制、工程实践和常见问题。"
...
> 更新说明：Gulp 现在更适合处理文件流式任务，例如复制目录、批量压缩图片、生成雪碧图、处理静态站点资源、兼容老项目构建流程等。新前端应用的模块打包一般优先选择 Vite、Webpack、Rspack/Rsbuild、Rollup 等工具，不建议再用 Gulp 承担完整应用打包职责。

适合继续使用 Gulp 的场景：

- 没有复杂模块依赖，只需要对文件做批处理。
- 历史项目已经有稳定的 Gulp 流程，迁移成本高。
- 小程序、静态站点或多目录复制场景，需要灵活控制输入输出目录。
- 作为主构建工具之外的辅助脚本，例如图片压缩、文档复制、版本文件生成。
