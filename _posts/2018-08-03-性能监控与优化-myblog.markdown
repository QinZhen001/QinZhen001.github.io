...
layout:     post
title:      "性能监控与优化"
description: "围绕「性能监控与优化」整理核心知识点、实践经验、常见问题和参考资料，便于持续查阅。"
...
现代前端性能监控建议以 Core Web Vitals 为核心，同时结合业务指标和错误监控。FID 已经被 INP 替代，当前更应该关注用户交互整体响应质量。

| 指标 | 含义 | 优化方向 |
| --- | --- | --- |
| LCP | 最大内容绘制，衡量主要内容加载速度 | 优化服务端响应、关键资源、图片、字体、渲染阻塞资源 |
| INP | 交互到下次绘制，衡量整体交互响应 | 减少长任务、拆分 JS、降低主线程压力、优化事件回调 |
| CLS | 累积布局偏移，衡量视觉稳定性 | 图片/广告预留尺寸、避免插入式内容、控制字体切换 |
| TTFB | 首字节时间，衡量服务端和网络响应 | CDN、缓存、SSR 性能、接口耗时 |
| FCP | 首次内容绘制，衡量页面开始呈现 | 减少阻塞资源、优化 HTML/CSS/字体 |

监控系统不要只采集平均值，更要关注 P75/P90/P95、设备类型、网络类型、页面路由、版本号和地域分布。
...
50这个数值来自来自于 Google 的 [RAIL 模型](https://web.dev/articles/rail)
...
new PerformanceObserver((list) => {
  list.getEntries().forEach((entry) => {
    ...
    console.log(`Total duration: ${duration}`);   // 从事件发生（如用户点击）到浏览器完全处理完毕的总耗时
    console.log(`Event delay: ${delay}`);  // 事件触发后，浏览器主线程开始处理该事件前的等待时间。(此延迟可能由长任务或主线程被其他代码阻塞引起)
    console.log(`Event handler duration: ${eventHandlerTime}`);   // 浏览器同步执行与该事件关联的所有处理函数（如 clickhandler）所花费的实际时间
  });
}).observe({ type: "event" });
```
...
| 优点         | 说明                                                |
| :----------- | :-------------------------------------------------- |
| 减小打包体积 | 库代码不打入业务 bundle，首屏 JS 体积更小           |
| CDN 缓存     | 可利用 CDN 缓存，用户可能已缓存过该资源（跨站共享） |
| 独立更新     | 可以独立更新库版本，无需重新构建部署业务代码        |
| 并行加载     | 浏览器可与业务代码并行下载                          |
| 简单快速     | 无需构建配置，直接在 HTML 中添加即可                |

缺点：

| 缺点            | 说明                                      |
| :-------------- | :---------------------------------------- |
| 额外网络请求    | 多一个 HTTP 请求，有网络延迟风险          |
| 全局污染        | 挂载到 window，可能造成命名冲突           |
| 版本管理困难    | 难以精确控制版本，依赖 CDN 可用性         |
| 无 Tree Shaking | 必须加载整个库，无法按需引入              |
| 类型支持差      | TypeScript 需要额外声明或只能用 type 导入 |
| 依赖顺序问题    | 需确保脚本加载顺序正确                    |
...
| 优点         | 说明                                         |
| :----------- | :------------------------------------------- |
| Tree Shaking | 只打包用到的代码，减少无用代码               |
| 版本锁定     | package-lock.json / yarn.lock 精确控制版本   |
| 类型完整     | 天然支持 TypeScript，IDE 智能提示完整        |
| 模块化       | ES Module 方式，无全局污染                   |
| 统一管理     | 依赖统一在 package.json 管理，便于审计和升级 |
| 离线可用     | 代码打包在 bundle 中，不依赖外部 CDN         |

缺点：

| 缺点           | 说明                                |
| :------------- | :---------------------------------- |
| 增加打包体积   | 库代码打入 bundle，增大首屏 JS 大小 |
| 构建时间       | 需要参与构建，可能增加构建时间      |
| 更新需重新部署 | 升级库版本需重新构建发布            |
| 无法跨站缓存   | 每个站点独立打包，无法共享缓存      |

##### 实际场景选择建议

| 场景                             | 推荐方式                     |
| :------------------------------- | :--------------------------- |
| 大型通用库（React、Vue、Lodash） | Script（CDN），利用缓存优势  |
| 业务强相关的小型库               | NPM，便于管理和 Tree Shaking |
| 需要精确类型支持                 | NPM                          |
| 快速原型/测试                    | Script                       |
| 对首屏性能敏感                   | Script（并行加载）           |
| 内网/离线环境                    | NPM                          |
...
* 优点：这个上报库可以独立于业务代码更新，且不增加主 bundle 体积。
* 缺点：需要在每个 HTML 文件中手动添加，且 TypeScript 只能用 import type 方式。

#### 链接并发

HTTP/1.1 协议  每个域名（hostname）通常限制为 6-8 个并发链接 （这里可能会有性能问题，导致请求queueing）
...
核心原因：FPS 是**平均值**，掩盖了**帧间抖动**

一个经典反例（Apple WWDC'18）

| 场景 A                                       | 场景 B                                  |
| -------------------------------------------- | --------------------------------------- |
| 平均 **40 FPS**                              | 平均 **30 FPS**                         |
| FrameTime: 16, 16, 16, **117**, 16, 16... ms | FrameTime: 33, 33, 33, 33, 33, 33... ms |
| 单次长卡顿 **117ms**                         | 每帧稳定 33ms                           |
| 😵 观感：**卡顿**（micro stuttering）         | 😊 观感：**平滑**                        |

**结论**：FPS 更高的 A 比 FPS 更低的 B 感觉更卡。

三个底层机制

**1. 视觉惯性（Visual Inertia）**

人眼看到连续 3 帧相同节奏后，会潜意识假设"下一帧也这么快"。当 60fps 突然掉到 25fps，这个**突变**被大脑感知为"顿了一下"，即使平均 FPS 仍有 55。

**2. 电影帧理论（24fps 临界点）**

电影以 24fps 播放（单帧 41.67ms），这是人眼辨别"画面不连续"的下限。**任何单帧超过 83ms（2 倍电影帧）就会被明确感知为卡顿**，哪怕整体平均 FPS 很高。

**3. VSync 对齐丢失**

屏幕以固定频率刷新（60Hz = 16.67ms，120Hz = 8.33ms）。如果一帧计算超过 VSync 间隔，就会**丢帧**——屏幕在那个 VSync 上只能显示旧画面，用户就看到"停顿"。FPS 统计会把这个丢帧摊进 1 秒均值，从数字上看不出来。

#### 流畅度判断

🎯 核心原则

**不看平均 FPS，看 FrameTime 的分布和最大值。**

✅ 推荐的判定指标（按优先级）

1. **最大 FrameTime / 长帧数**（最重要）

```text
✅ 健康：所有帧都 < 33ms（等于 30 FPS 下限）
⚠️  警告：出现 > 50ms 的帧
❌ 卡顿：出现 > 83ms 的帧（PerfDog-Jank 阈值）
🚨 严重：出现 > 125ms 的帧（BigJank 阈值）
```

2. **PerfDog-Jank 算法**（借鉴移动端标准）

同时满足两个条件计 1 次 Jank：

```text
① 当前帧 FrameTime > 前 3 帧平均 × 2
② 当前帧 FrameTime > 83.33ms
```

3. **FrameTime 方差 / P95 / P99**

- P95 FrameTime < 20ms → 良好
- P99 FrameTime < 33ms → 良好
- 方差越小越好（抖动小）
