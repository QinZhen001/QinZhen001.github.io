---
layout:     post
title:      "CSS 基础知识与常见实践总结"
description: "围绕「CSS 基础知识与常见实践总结」梳理基础概念、关键属性、实践技巧和常见问题。"
...
`:has()` 可以根据后代、兄弟或表单状态反向影响当前元素，常被称为“父选择器”能力，但它不是无限制的父级回溯语法。实际使用时要注意选择器复杂度，避免在大范围节点上写过重的 `:has()`。
...
## padding-top 百分比实现响应式背景图

> 原文：padding-top(percentage)实现响应式背景图片（2017-11-29 23:12:00）
