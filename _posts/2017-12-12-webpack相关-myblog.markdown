---
layout:     post
title:      "Webpack 核心概念、配置与优化总结"
description: "系统整理「Webpack 核心概念、配置与优化总结」的配置方法、工作机制、工程实践和常见问题。"
...
- **定义**: 在Webpack中，模块是能够被Webpack处理的任何文件。常见的模块类型包括JavaScript文件、CSS文件、图片等。
- **特点**: 模块是Webpack构建过程的基本单位，它可以导入其他模块，并且有自己的依赖关系。

Chunk

- **定义**: Chunk是Webpack在构建过程中生成的代码块，它可以包含一个或多个模块。
- **特点**: Chunks用于组织和分离代码，以便能够实现代码分拆（code splitting）。例如，异步加载的模块通常会被放在单独的Chunk中，以优化加载性能。

Bundle

- **定义**: Bundle是最终生成的一个或多个文件，通常是JavaScript文件，其中打包了一个或多个Chunks。
- **特点**: Bundles是Webpack生成的最终输出，浏览器下载和执行的就是这些Bundle。一个应用可能会包含多个Bundle，例如主Bundle和懒加载的Bundle。

总结

- **Module** 是最小的构建单位，可以是任何代码文件。
- **Chunk** 是由Webpack在构建过程中生成的代码块，由一个或多个模块组成。
- **Bundle** 是最终生成的文件，作为应用的输出，通常包括一个或多个Chunks。
...
> 原文：Source Map详解（2018-03-31 23:46:00）
