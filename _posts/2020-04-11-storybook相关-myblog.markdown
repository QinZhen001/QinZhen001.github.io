...
早期版本需要手动指定 Webpack 5 builder，新版本通常通过 framework 包隐式决定 builder。选择思路：

- React + Vite：优先 `@storybook/react-vite`。
- Vue 3 + Vite：优先 `@storybook/vue3-vite`。
- 历史 Webpack 项目：继续使用对应 Webpack framework，并把项目里的别名、loader、静态资源规则同步到 Storybook。
