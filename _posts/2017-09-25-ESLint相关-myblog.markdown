...
description: "系统整理「ESLint 配置与规则实践总结」的配置方法、工作机制、工程实践和常见问题。"
...
typescript-eslint/parser是 ESLint 用于解析 TypeScript 代码的解析器。它允许 ESLint 理解 TypeScript 语法，并与 @typescript-eslint/eslint-plugin插件配合，为 TypeScript 代码提供代码检查和规则。

核心作用

- **将 TypeScript 代码转换为 ESLint 可识别的 AST**（抽象语法树）。
- **启用 TypeScript 专属的 ESLint 规则**（如类型检查、TS 语法规范等）。
- 替代 ESLint 默认的解析器（`espree`），以支持 TS 特有的语法（如接口、泛型、装饰器等）。

配置 ESLint

在 `.eslintrc.js`（或 `.eslintrc.json`、`package.json`的 `eslintConfig`字段）中配置：
...
module.exports = {
  ...
  parserOptions: {
    ecmaVersion: 2020,           // 支持最新 ECMAScript
    sourceType: 'module',        // 使用 ES 模块
    project: './tsconfig.json',  // 指定 tsconfig 文件（可选，用于类型感知规则）
  },
  plugins: ['@typescript-eslint'], // 启用 TS 规则插件
  extends: [
    'eslint:recommended',
...
> 注意：类型检查会降低 ESLint 运行速度，且需要正确的 `tsconfig.json`配置。

1. 启用类型检查规则

在 `parserOptions`中指定 `project`字段，并添加 `plugin:@typescript-eslint/recommended-type-checked`（或 `strict-type-checked`）：
...
### @typescript-eslint/base和@typescript-eslint/recommended

plugin:@typescript-eslint/base:

* 这是最基础的配置，它设置了正确的解析器（@typescript-eslint/parser）和基本的解析器选项，但不启用任何具体的规则。**它只确保 ESLint 能够正确解析 TypeScript 代码。**
* 使用这个配置意味着你需要自己手动启用所有需要的规则，或者从其他扩展中继承规则。它相当于一个空的起点，只提供了解析 TypeScript 的能力。

plugin:@typescript-eslint/recommended:

* 这个配置扩展了 base，并额外启用了一系列推荐的规则。这些规则是经过精心挑选的，旨在捕捉 TypeScript 代码中常见的问题，同时避免过于严格或争议性的规则。
* 例如，它启用了 @typescript-eslint/no-explicit-any、@typescript-eslint/explicit-function-return-type（在某些版本中可能不是）等。具体规则列表可以参考官方文档。

---

总结区别：

- `base`只提供了解析 TypeScript 的基础设施，没有规则。
- `recommended`在 `base`的基础上增加了一组推荐规则。
