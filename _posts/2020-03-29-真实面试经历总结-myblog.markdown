...
* 狂问项目。项目架构，项目难点，项目优化
* web worker 会跨域吗？ **如果要传递很大数据怎么办？**  (传递很大数据  => 这样纸在传递过程中会耗时很大，感觉上web worker的意义也不大了)  （可以尝试SharedArrayBuffer SharedWorker）
...
* **h5 和移动端通信 除了url schema 还有别的方式吗？**怎么改变url =>  通过 iframe 
