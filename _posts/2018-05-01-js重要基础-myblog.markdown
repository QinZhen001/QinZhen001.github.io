---
layout:     post
title:      "JavaScript 重要基础与异步编程总结"
description: "系统整理「JavaScript 重要基础与异步编程总结」的语言特性、底层机制、代码示例和实践经验。"
...
* **浏览器**：`globalThis` 指向 `window` 对象
* **Node.js**：指向 `global` 对象
* **Web Workers**：指向 `self` 对象
* **其他环境**：如 Deno、物联网设备等，均通过 `globalThis` 提供一致的全局对象访问方式
...
[https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/AggregateError](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/AggregateError)
...
[Reflect 大全](https://juejin.cn/post/7419978042246365221)
...
[https://github.com/protobufjs/protobuf.js/blob/HEAD/cli/README.md#pbjs-for-javascript](https://github.com/protobufjs/protobuf.js/blob/HEAD/cli/README.md#pbjs-for-javascript)
...
1. **安全性**:
   - `crypto.getRandomValues` 是加密安全的，适用于需要高安全性要求的场景。
   - `Math.random` 不是加密安全的，不适用于需要高安全性要求的场景。

2. **随机性**:
   - `crypto.getRandomValues` 生成的随机数质量高，随机性强。
   - `Math.random` 生成的随机数质量一般，随机性较低。

3. **适用场景**:
   - `crypto.getRandomValues` 适用于加密、身份验证、生成安全令牌等需要高质量随机数的场景。
   - `Math.random` 适用于一般的随机数生成场景，如随机选择、生成随机颜色等。

选择指南：

- **需要高安全性和高质量随机数**: 使用 `crypto.getRandomValues`。
- **一般随机数生成**: 使用 `Math.random`。
