---
layout:     post
title:      "Webpack Loader 机制与常用 Loader 总结"
description: "系统整理「Webpack Loader 机制与常用 Loader 总结」的配置方法、工作机制、工程实践和常见问题。"
...
> 更新说明：Webpack 5 已经内置 Asset Modules，很多过去依赖 `file-loader`、`url-loader`、`raw-loader` 的资源处理场景可以直接用 `type: 'asset/resource'`、`type: 'asset/inline'`、`type: 'asset/source'` 或 `type: 'asset'` 替代。历史项目可以继续维护旧 loader，新项目建议优先使用内置资源模块。
